/*  
 * JOB 2 : Membaca push button ke mqtt
...
void loop() {
  ...
  if (data_send_time > data_send_period){
    ...
    if (button1_status == 1){
      client.publish("trainer_iot_upi_01/button1","on");
    } else {
      client.publish("trainer_iot_upi_01/button1","off");
    }

    if (button2_status == 1){
      client.publish("trainer_iot_upi_01/button2","on");
    } else {
      client.publish("trainer_iot_upi_01/button2","off");
    }
